...
function requireConfig() {
  return new Promise(resolve => {
    ...
    if ($.isNode()) {
      ...
      if ($.shareCodesArr.length > cookiesArr.length) { if (process.env.JDFACTORY_FORBID_ACCOUNT) process.env.JDFACTORY_FORBID_ACCOUNT.split('&').map((item, index) => Number(item) === 0 ? $.shareCodesArr = [] : $.shareCodesArr.splice(Number(item) - 1 - index, 1)) }
    }
